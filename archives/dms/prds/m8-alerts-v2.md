...
**Version:** v2
**Date:** 2026-05-20
**Scope:** Milestones 1–3, 5–7
**Definition:** Alert = deviation from desired operating behaviour. Not error logs — actionable signals to a human.
**Changes from v1:** +23 alerts. Added M7 section (8 alerts). Added M2/M3/M5/M6 alerts surfaced during gap-fill PRD rewrites. All 85 v1 alerts retained unchanged.
...
| A2.12 | **Geography Batch Sync Processing Failure** | Batch of geography change webhooks received but not fully processed within threshold | Tech | P1 |
...
| A3.14 | **Aadhaar/DL Edit Attempted Post-Creation** | Edit attempted on locked identity field (Aadhaar or DL) for an existing driver record | Tech | P1 |
| A3.15 | **Maker-Checker Checker Pool Down to Single Member** | Only one eligible Checker remains → single point of failure; one absence = pool outage | Tech + HO Central Logistics | P1 |
...
| A5.3 | **Sort Identifier Code Format Violation** | Generated code doesn't match expected pattern | Tech | P1 |
...
| A5.12 | **Multi-Box AWB Count Mismatch** | OMS requested N AWBs for N-box order but fewer than N AWBs successfully generated | Tech + Ops | P0 |
| A5.13 | **Dual AWB Active During Transition Period** | Shipsy AWB and Locus AWB both non-cancelled for same order_id simultaneously | Tech | P0 |
| A5.14 | **Sort Code Blocked Locus Ingestion** | AWB generated with Sort Identifier Code that passes M5 validation but fails M6 ingestion format check | Tech | P1 |
...
| A6.44 | **Locus Ingestion Circuit Breaker Opened** | Full Locus ingestion service unreachable; circuit breaker tripped after 5 consecutive connection failures in 60s | Tech | P0 |
| A6.45 | **Pending Ingestion Queue Spike** | "Pending Ingestion" queue exceeds N orders while circuit breaker is open | Tech + Ops | P0 |
...
| A6.47 | **Parked Order Held >7 Days** | Order in Parked/Hold state beyond 7 days without cancellation — secondary escalation | HO Central Logistics | P0 |
...
| A6.51 | **Unallocated Orders Persisting N+ Dispatch Windows** | Same orders remain in Unallocated queue across N consecutive dispatch windows → chronic capacity gap | Ops + Central Logistics | P1 |
...
| A6.46 | **Concurrent Plan Edit Conflict** | Dispatcher received version-token conflict error — plan was modified by another user between their read and submit | Ops | P1 |
| A6.48 | **Mid-Mile Complete Override Without Full Scan** | Runner marked mid-mile trip Complete using manual override; one or more orders in batch have no DC scan record | Tech + Ops | P0 |
| A6.49 | **Plan Rollback Executed** | Plan reverted to a prior version by HO Central Logistics | HO Central Logistics + Tech | P1 |
| A6.50 | **SLA Risk Flag Overridden Without Acknowledgement** | Plan finalised with active SLA risk flag but no dispatcher acknowledgement record exists | Tech + Ops | P1 |
| A6.52 | **Hub Invalid on Active Mid-Mile Trip** | Mid-mile trip is in-progress but the hub it references no longer exists in Geography | Tech + Ops | P0 |
...
## M7 — Driver App & Execution Setup

| # | Alert | Trigger | Recipient | Severity |
|---|-------|---------|-----------|----------|
| A7.1 | **Driver Offline During Active Trip >X Minutes** | Driver app connectivity lost for longer than threshold while trip is in Open/In-Progress state | Dispatcher + Ops | P1 |
| A7.2 | **Driver App Crash During Active Trip** | App crash signal received while driver has active trip; delivery sequence state unknown | Tech + Ops | P0 |
| A7.3 | **Offline Event Queue Overflow** | Driver's offline-mode event buffer exceeds capacity threshold; events at risk of loss on reconnect | Tech | P1 |
| A7.4 | **Multi-Device Login Conflict** | Same driver authenticated and active on two devices simultaneously | Tech + Ops | P0 |
| A7.5 | **Recompute Silent App Fail — General** | Backend route recompute succeeded (any trigger: cancellation, reassignment, breakdown) but driver app not notified within threshold | Tech | P0 |
| A7.6 | **Driver App Session Active After Termination/Suspension** | Driver status changed to Terminated or Suspended but their Locus app session remains authenticated | Tech | P0 |
| A7.7 | **Earnings Calculation Mismatch** | Driver-visible earnings in app differ from system-calculated earnings by >X% | Tech + Ops | P1 |
| A7.8 | **Trip Marked Complete Without Final Delivery Scan** | Trip status updated to Complete but one or more deliveries have no scan record in Locus | Tech + Ops | P1 |

---

## Summary

| Milestone | v1 Count | New in v2 | v2 Count |
|-----------|----------|-----------|----------|
| M1 — RBAC | 7 | 0 | 7 |
| M2 — Geography | 11 | 1 | 12 |
| M3 — Driver Module | 13 | 2 | 15 |
| M5 — Order Sorting | 11 | 3 | 14 |
| M6 — Planning Engine | 43 | 9 | 52 |
| M7 — Driver App | 0 | 8 | 8 |
| **Total** | **85** | **+23** | **108** |
...
*(Q1–Q11 carried forward from v1; Q12–Q15 new in v2)*
...
| Q12 | Pending Ingestion queue spike threshold (N orders) for circuit breaker alert | A6.45 |
| Q13 | Unallocated orders persistence threshold (N consecutive windows) before chronic-gap alert | A6.51 |
| Q14 | Driver offline duration threshold (X minutes) before A7.1 fires | A7.1 |
| Q15 | Earnings mismatch threshold (X%) before A7.7 fires | A7.7 |
