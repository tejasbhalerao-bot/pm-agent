...
| # | Change |
|---|---|
| 1 | UC5.3: Deactivation now explicitly drops all accrued but unpaid earnings for affected drivers |
| 2 | UC6.2: Clarified no approval gate — Locus passes computed earnings directly to Truemeds for reconciliation |
| 3 | Ad-hoc / Festival Based: Added mid-window salary structure switch behaviour — accrue earnings up to switch date, drop remainder |
| 4 | Removed navigation step placeholders (deferred) |
| 5 | Removed Open Questions section — all questions resolved |
...
At the end of each completed shift, Locus calculates the driver's total earnings and passes the computed record directly to Truemeds systems. There is no approval gate — the record is consumed as-is for financial reconciliation.
