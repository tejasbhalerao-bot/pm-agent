...
**On-Time Delivery** (did we dispatch and route on schedule) and **OP2OD Uplift** (did the order actually get delivered) are kept as separate nodes: a delivery can be on time and still fail, or succeed while late. The roadmap below is organized by shipping quarter, not by this loop's own structure — but every initiative in it traces back to a specific node or leak above.
